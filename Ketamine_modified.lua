...
do -- Set properties
    ...
    objects["InstanceOpenBtnLabel"]["Size"] = UDim2.new(1, 0, 1, 0);
    objects["InstanceOpenBtnLabel"]["Position"] = UDim2.new(0, 0, 0, 0);
...
-- Функция скрыть GUI и показать плавающую кнопку
local function hideGui()
    ...
    script.Parent.Visible = false
    openBtn.Visible = true  -- показать плавающую кнопку
...
game:GetService("UserInputService").InputChanged:Connect(function(input)
    if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        ...
        local newX = btnStartPos.X.Scale + delta.X / vp.X
        local newY = btnStartPos.Y.Scale + delta.Y / vp.Y
...
-- Клик по плавающей кнопке — открыть GUI (обработка через InputEnded выше)
-- MouseButton1Click оставляем пустым чтобы не дублировать
